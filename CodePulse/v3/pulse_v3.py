...
class Sine(AtomSegment):
    default_name = 'sine'
    param_lbls = ['frequency', 'amplitude', 'phase']
    def __init__(self, name=default_name,
                 freq_amp_phase=(.0,.0,.0,), 
                 duration=.0):
        # type: str, tuple[float,float,float], float
        super(Sine, self).__init__(name, freq_amp_phase, duration, self.param_lbls)
    def getWave(self, sample_rate):
        freq, amp, phase = self.get('frequency'), self.get('amplitude'), self.get('phase')
...
class Segment():
    ...
    def getWave(self, sample_rate, marker_low_high=(0,1), normalize=False):
        ...
        # build timestep
        timestep = np.linspace(0, self.getDuration()/sample_rate, int(len(wave)))
        # handle normalize
        if normalize:
    ...
    def makeSequence(self, repeat, constant_slope=0, compensate=0, wait_time=0, name=''):
        ...
        for i in range(repeat):
            ...
            if len(constant_slope) > 0 :
                for atom_name, slope in constant_slope:
                    new_seg.constantToRamp(atom_name, constant_slope)
            if compensate != 0:
...
class Sequence(object):
    ...
    def __init__(self, name, config=''):
        ...
        self.segments = []
        self.segments_names = []
        self.config = config # track how this sequence was generated 
    ...
    def getWave(self, sample_rate, marker_low_high=(0,1), normalize=False):
        ...
        timestep = np.linspace(0, self.getDuration()/sample_rate, int(len(full_wave)))
