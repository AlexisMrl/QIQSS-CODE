...
def loadmapDC(lst,Temp_name,Vg_I_index):
	...
	for y in lst:
		...
		for line in datatemp.comment:
			...
			match2=re.search("Vds:([^;]+)",line)
			if match:
				datatemp.T=float(re.sub('\n','',match.group((1))))
			if match2:
				datatemp.Vds=float(re.sub('\n','',match2.group((1))))
		if datatemp.Vds==0:
			print('Carefull : no Vds found in comment. We assume 2D measurement with Vds in first row')
			datatemp.Vds=datatemp.value[0][0,0]
		...
		if i.Vds==0.9:
			...
			for rep in range(min,max,2):
				...
				if courbefit[1]!=nan:
					...
					ssi900=np.append(ssi900,i.value[4][0][rep])
			plt.figure(15)
